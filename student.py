...
class studentClass:
    def __init__(self, root):
        ...
        # ==== Widgets ====
        #======column1======
        lb1_roll = Label(self.root, text="Roll No.", font=("goudy old style", 15, 'bold'), bg='white')
        lb1_roll.place(x=10, y=60)

        lb1_Name = Label(self.root, text="Name", font=("goudy old style", 15, 'bold'), bg='white')
        lb1_Name.place(x=10, y=100)

        lb1_Email = Label(self.root, text="Email", font=("goudy old style", 15, 'bold'), bg='white')
        lb1_Email.place(x=10, y=140)

        lb1_gender = Label(self.root, text="Gender", font=("goudy old style", 15, 'bold'), bg='white')
        lb1_gender.place(x=10, y=180)

        lb1_state = Label(self.root, text="State", font=("goudy old style", 15, 'bold'), bg='white')
        lb1_state.place(x=10, y=220)
        ...
        lb1_city = Label(self.root, text="City", font=("goudy old style", 15, 'bold'), bg='white')
        lb1_city.place(x=310, y=220)

        txt_state = Entry(self.root,textvariable=self.var_city, font=("goudy old style", 15, 'bold'), bg='lightyellow')
        txt_state.place(x=380, y=220,width=100)





        lb1_pin = Label(self.root, text="Pin", font=("goudy old style", 15, 'bold'), bg='white')
        lb1_pin.place(x=500, y=220)
        ...
        lb1_address = Label(self.root, text="Address", font=("goudy old style", 15, 'bold'), bg='white')
        lb1_address.place(x=10, y=260)
        ...
        #=============Column2==================
        lb1_dob = Label(self.root, text="D.O.B", font=("goudy old style", 15, 'bold'), bg='white')
        lb1_dob.place(x=360, y=60)

        lb1_contact = Label(self.root, text="Contact", font=("goudy old style", 15, 'bold'), bg='white')
        lb1_contact.place(x=360, y=100)

        lb1_admission = Label(self.root, text="Admission", font=("goudy old style", 15, 'bold'), bg='white')
        lb1_admission.place(x=360, y=140)

        lb1_course = Label(self.root, text="Course", font=("goudy old style", 15, 'bold'), bg='white')
        lb1_course.place(x=360, y=180)
        ...
        lb1_search_roll = Label(self.root,text="Roll No.", font=("goudy old style", 15, 'bold'), bg='white')
        lb1_search_roll.place(x=720, y=60)
        ...
        self.CourseTable.column("pin", width=70)
    ...
    def add(self):
        ...
        try:
            if self.var_roll.get() == "":
                ...
                row = cur.fetchone()
                if row is not None:
                    messagebox.showerror("Error", "This Roll Number already present", parent=self.root)
                else:
        ...
        try:
            if self.var_roll.get() == "":
                ...
                if row is None:
                    ...
                    cur.execute("""UPDATE student SET name=?, email=?, gender=?, dob=?, contact=?, admission=?,
                            course=?, state=?, city=?, pin=?, address=? WHERE roll=?""", (
                        self.var_name.get(),
                        self.var_email.get(),
                        self.var_gender.get(),
                        self.var_dob.get(),
